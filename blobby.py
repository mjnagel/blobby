...
# Check if both --yell and --cheer are provided
if args.yell and args.cheer:
    print("Error: Only one of --yell or --cheer can be provided, not both.")
    parser.print_help()
    exit(1)
...
if args.cheer:
    ...
    for frame in ImageSequence.Iterator(img):
        ...
        frames.append(process_frame(frame, logo, IMGSIZE, LOGO_SIZE))
    ...
    if args.show:
        ...
        print("Blobified GIF saved to " + outfile)
else:
    # Process PNG
    imgfinal = Image.new("RGBA", (IMGSIZE, IMGSIZE))
    imgbase = Image.open(BLOB_FILE).convert("RGBA")
    imgbase.thumbnail((IMGSIZE, IMGSIZE))
    imgfinal.paste(imgbase, (int((IMGSIZE - imgbase.width) / 2), int((IMGSIZE - imgbase.height) / 2)), imgbase)

    # Add Logo
    logo = Image.open(args.image).convert("RGBA")
    logo.thumbnail(LOGO_SIZE)
    imgfinal.paste(logo, (0, 0), logo)
